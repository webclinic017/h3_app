...
@immutable
class OrderItemPromotion extends Equatable {
  ///订单ID
  String id = "";
  ...
  String get displayReason {
    String result = "";
    if (this.promotionType == PromotionType.Gift) {
      result = sprintf("%s%s", ["(赠)", this.reason]);
    } else if (this.promotionType == PromotionType.ProductBargain ||
        this.promotionType == PromotionType.OrderBargain) {
      result = sprintf("%s%s", ["(议)", this.reason]);
    } else if (this.promotionType == PromotionType.ProductDiscount ||
        this.promotionType == PromotionType.OrderDiscount) {
      result = sprintf("%s%s", ["(折)", this.reason]);
    }

    return result;
  }

  factory OrderItemPromotion.newOrderItemPromotion() {
  ...
  ///转List集合
  static List<OrderItemPromotion> toList(List<Map<String, dynamic>> lists) {
    var result = new List<OrderItemPromotion>();
    lists.forEach((map) => result.add(OrderItemPromotion.fromJson(map)));
    return result;
  }

  factory OrderItemPromotion.fromJson(Map<String, dynamic> map) {
    return OrderItemPromotion()
  ...
  factory OrderItemPromotion.clone(OrderItemPromotion obj) {
    return OrderItemPromotion()
      ..id = obj.id
  ...
  String toDescString() {
    return "${StringUtils.isBlank(this.reason) ? this.promotionType.name : this.reason}优惠[$this.discountAmount]";
  }

...
class DesUtils {
  static final _key = '\$AZKOSS\$'; //加密
  static final _iv = [1, 3, 7, 3, 3, 8, 2, 7];

  ///加密
  static String encrypt(String data) {
    var content = utf8.encode(data);
    DES des = DES(key: utf8.encode(_key), mode: DESMode.CBC, iv: _iv);
    var encrypted = des.encrypt(des.pad(content));

    return base64.encode(encrypted).replaceAll('+', '-').replaceAll('/', '_').replaceAll('=', '.');
  }

  ///加密
  static String decrypt(String data) {
    print(">>>>>$data");
    var content = data.replaceAll('-', '+').replaceAll('_', '/').replaceAll('.', '=');

    print(">>>>>$content");

    print(">>>>>${base64.decode(content)}");

    DES des = DES(key: utf8.encode(_key), mode: DESMode.CBC, iv: _iv);
    List<int> k = des.decrypt(base64.decode(content));
    var u = des.unpad(k);
...
class _BaseDES {
  ...
  _BaseDES(this.key, this.mode, this.iv) {
    if (iv != null && iv.length != 8) {
      throw Exception('Invalid Initial Value (iv), must be a multiple of ${DES.BLOCK_SIZE} bytes');
    }
  }
}
...
class DES {
  ...
  static const _pc1 = [56, 48, 40, 32, 24, 16, 8, 0, 57, 49, 41, 33, 25, 17, 9, 1, 58, 50, 42, 34, 26, 18, 10, 2, 59, 51, 43, 35, 62, 54, 46, 38, 30, 22, 14, 6, 61, 53, 45, 37, 29, 21, 13, 5, 60, 52, 44, 36, 28, 20, 12, 4, 27, 19, 11, 3];

  // number left rotations of pc1
  static const _leftRotations = [1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1];

  // permuted choice key (table 2)
  static const _pc2 = [13, 16, 10, 23, 0, 4, 2, 27, 14, 5, 20, 9, 22, 18, 11, 3, 25, 7, 15, 6, 26, 19, 12, 1, 40, 51, 30, 36, 46, 54, 29, 39, 50, 44, 32, 47, 43, 48, 38, 55, 33, 52, 45, 41, 49, 35, 28, 31];

  // initial permutation IP
  static const _ip = [57, 49, 41, 33, 25, 17, 9, 1, 59, 51, 43, 35, 27, 19, 11, 3, 61, 53, 45, 37, 29, 21, 13, 5, 63, 55, 47, 39, 31, 23, 15, 7, 56, 48, 40, 32, 24, 16, 8, 0, 58, 50, 42, 34, 26, 18, 10, 2, 60, 52, 44, 36, 28, 20, 12, 4, 62, 54, 46, 38, 30, 22, 14, 6];

  // Expansion table for turning 32 bit blocks into 48 bits
  static const _expansionTable = [31, 0, 1, 2, 3, 4, 3, 4, 5, 6, 7, 8, 7, 8, 9, 10, 11, 12, 11, 12, 13, 14, 15, 16, 15, 16, 17, 18, 19, 20, 19, 20, 21, 22, 23, 24, 23, 24, 25, 26, 27, 28, 27, 28, 29, 30, 31, 0];
  ...
  static const _p = [15, 6, 19, 20, 28, 11, 27, 16, 0, 14, 22, 25, 4, 17, 30, 9, 1, 7, 23, 13, 31, 26, 2, 8, 18, 12, 29, 5, 21, 10, 3, 24];

  // final permutation IP^-1
  static const _fp = [39, 7, 47, 15, 55, 23, 63, 31, 38, 6, 46, 14, 54, 22, 62, 30, 37, 5, 45, 13, 53, 21, 61, 29, 36, 4, 44, 12, 52, 20, 60, 28, 35, 3, 43, 11, 51, 19, 59, 27, 34, 2, 42, 10, 50, 18, 58, 26, 33, 1, 41, 9, 49, 17, 57, 25, 32, 0, 40, 8, 48, 16, 56, 24];
  ...
  List<int> _convertToBits(List<int> data) {
    List<int> result = [];
    data.forEach((e) => result += e.toRadixString(2).padLeft(8, '0').codeUnits.map((el) => el - 48).toList());
    return result;
  }
  ...
  List<int> _permutate(List<int> table, List<int> block) => table.map((e) => block[e]).toList();
  ...
  List<int> _desCrypt(List<int> block, int cryptType) {
    ...
    for (int i = 0; i < 16; i++, iteration += iterationAdjustment) {
      ...
      List<List<int>> B = [right.sublist(0, 6), right.sublist(6, 12), right.sublist(12, 18), right.sublist(18, 24), right.sublist(24, 30), right.sublist(30, 36), right.sublist(36, 42), right.sublist(42)];
  ...
  List<int> crypt(List<int> data, int cryptType) {
    ...
    if (data.length % BLOCK_SIZE != 0) {
      if (cryptType == _DECRYPT) {
        ...
        throw Exception('Invalid data length, data must be a multiple of $BLOCK_SIZE bytes.');
      }
    }
    if (_baseDES.mode == DESMode.CBC) {
      if (_baseDES.iv != null) {
        ...
        throw Exception('For CBC mode, you must supply the Initial Value (IV) for ciphering');
      }
    }
  ...
  DES3({List<int> key, this.mode = DESMode.ECB, this.iv = DES.IV_ZEROS}) {
    ...
    if (key.length != keySize) {
      if (key.length == 16) {
        ...
        throw Exception('Invalid triple DES key size. Key must be either 16 or 24 bytes long');
      }
    }
    if (mode == DESMode.CBC) {
      if (iv == null) {
        // Use the first 8 bytes of the key
        iv = key.sublist(0, DES.BLOCK_SIZE);
      }

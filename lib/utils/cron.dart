...
class Schedule {
  ...
  factory Schedule({
    ...
    final parsedHours = parseConstraint(hours)?.where((x) => x >= 0 && x <= 59)?.toList();
    ...
    final parsedWeekdays = parseConstraint(weekdays)?.where((x) => x >= 0 && x <= 7)?.map((x) => x == 0 ? 7 : x)?.toSet()?.toList();
...
class _Cron implements Cron {
  ...
  void _scheduleNextTick() {
    ...
    final ms = (isTickMinute ? 60 : 1) * _millisecondsPerSecond - (now.millisecondsSinceEpoch % ((isTickMinute ? 60 : 1) * _millisecondsPerSecond));

...
class StoreTableArea extends BaseEntity {
  ///表名称
  static final String tableName = "pos_store_table_area";
  ...
  ///Map转实体对象
  factory StoreTableArea.fromMap(Map<String, dynamic> map) {
    return StoreTableArea()
      ..id = Convert.toStr(
  ...
  ///构建空对象
  factory StoreTableArea.newStoreTableArea() {
    return StoreTableArea()
  ...
  ///复制新对象
  factory StoreTableArea.clone(StoreTableArea obj) {
    return StoreTableArea()
      ..id = obj.id
  ...
  ///Map转List对象
  static List<StoreTableArea> toList(List<Map<String, dynamic>> lists) {
    var result = new List<StoreTableArea>();
    lists.forEach((map) => result.add(StoreTableArea.fromMap(map)));
    return result;
  }

...
class Decimal extends Number {
  ...
  Decimal(String value) {
    _rational = Rational.parse(value);
  }

  factory Decimal.parse(String value) => Decimal._fromRational(Rational.parse(value));

  factory Decimal.fromInt(int value) => Decimal._fromRational(Rational(BigInt.from(value)));

  factory Decimal.fromDouble(double value) => Decimal._fromRational(Rational.parse(value.toString()));
  ...
  @override
  Number operator %(Number other) => Decimal._fromRational(_rational % _convertToDecimal(other)._rational);
  ...
  @override
  Decimal operator /(Number other) => Decimal._fromRational(_rational / _convertToDecimal(other)._rational);

  @override
  Integer operator ~/(Number other) => Decimal._fromRational(_rational ~/ _convertToDecimal(other)._rational).toInteger();

  @override
  bool operator <(Number other) => _rational < _convertToDecimal(other)._rational;
  ...
  @override
  Integer get sign => new Integer(_rational.signum);
  ...
  @override
  Number remainder(Number other) => Decimal._fromRational(_rational.remainder(Rational.parse(other.toString())));

  @override
  Integer round() => Decimal._fromRational(_rational.round()).toInteger();

  @override
  Integer toInteger() => new Integer(_rational.toInt());
  ...
  Number roundWithPrecision(int precision, [RoundingMode mode = RoundingMode.nearestEven]) {
    if (precision >= 0) {
      switch (mode) {
  ...
  Number _roundTowardsZero(int precision) {
    ...
    tempNumber = (tempNumber >= Integer(0)) ? tempNumber.floor() : tempNumber.ceil();
  ...
  Number _roundFromZero(int precision) {
    final Integer multiplier = Integer(pow(10, precision));
    Number tempNumber = this * multiplier;
    tempNumber = (tempNumber >= Integer(0)) ? tempNumber.ceil() : tempNumber.floor();
    return tempNumber / multiplier;
  }
  ...
  Number _roundNearestOdd(int precision) {
    ...
    if (decimalPart.isNotEmpty && decimalPart[0] == '5') {
      int lastDigit = int.tryParse(integerPart[integerPart.length - 1]);
      return (lastDigit % 2 == 1) ? _roundTowardsZero(precision) : _roundFromZero(precision);
    } else {
      return _roundNearestFromZero(precision);
    }
  }
  ...
  Number _roundNearestDownward(int precision) {
    ...
    return (decimalPart.isNotEmpty && decimalPart[0] == '5') ? tempNumber.floor() / multiplier : _roundNearestFromZero(precision);
  }
  ...
  Number _roundNearestUpward(int precision) {
    ...
    return (decimalPart.isNotEmpty && decimalPart[0] == '5') ? tempNumber.ceil() / multiplier : _roundNearestFromZero(precision);
  }

  Number _roundNearestToZero(int precision) => (this >= Integer(0)) ? _roundNearestDownward(precision) : _roundNearestUpward(precision);
}
